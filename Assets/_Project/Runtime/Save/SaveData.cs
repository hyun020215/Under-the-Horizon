...
public sealed class SaveData
{
    ...
    public static SaveData FromState(GameState state) =>
        new()
        {
            ...
            discoveredEvidence = state.discoveredEvidence.ToList(),
            completedInteractions = state.completedInteractions.ToList(),
            completedPuzzles = state.completedPuzzles.ToList(),
            completedStoryScenes = state.completedStoryScenes.ToList(),
            completedObjectives = state.completedObjectives.ToList(),
            unlockedLocations = state.unlockedLocations.ToList(),
            puzzleProgress = state
                .puzzleProgress.Select(pair => new StringPair(pair.Key, pair.Value))
                .ToList(),
    ...
    public GameState ToState()
    {
        ...
        foreach (StringIntPair pair in trust ?? new())
            result.trust[pair.key] = pair.value;
        Add(result.flags, flags);
        Add(result.discoveredEvidence, discoveredEvidence);
        Add(result.completedInteractions, completedInteractions);
        Add(result.completedPuzzles, completedPuzzles);
        Add(result.completedStoryScenes, completedStoryScenes);
        Add(result.completedObjectives, completedObjectives);
        Add(result.unlockedLocations, unlockedLocations);
        foreach (StringPair pair in puzzleProgress ?? new())
            result.puzzleProgress[pair.key] = pair.value;
        return result;
    }

    private static void Add(HashSet<string> target, IEnumerable<string> values)
    {
        if (values == null)
            return;
