...
public sealed class GameStateStore : MonoBehaviour
{
    ...
    public void SetCurrentLocation(string locationId)
    {
        ...
        if (!string.IsNullOrEmpty(state.currentLocationId)) state.unlockedLocations.Add(state.currentLocationId);
        Notify();
    }
    ...
    public bool IsInteractionCompleted(string id) => state.completedInteractions.Contains(Normalize(id));
    ...
    public int GetTrust(string characterId) => state.trust.TryGetValue(Normalize(characterId), out int value) ? value : 0;
    ...
    public void ChangeAnxiety(int delta) { state.publicAnxiety = Mathf.Clamp(state.publicAnxiety + delta, 0, 100); Notify(); }
    public void ChangeIntegrity(int delta) { state.evidenceIntegrity = Mathf.Clamp(state.evidenceIntegrity + delta, 0, 100); Notify(); }
    public void SetPuzzleProgress(string id, string payload) { state.puzzleProgress[Normalize(id)] = payload ?? string.Empty; Notify(); }
    public bool TryGetPuzzleProgress(string id, out string payload) => state.puzzleProgress.TryGetValue(Normalize(id), out payload);
    ...
    private bool SetMembership(System.Collections.Generic.HashSet<string> set, string value, bool enabled)
    {
        string id = Normalize(value);
