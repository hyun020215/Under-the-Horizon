...
{
    [SerializeField]
    private ConditionGroupMode mode;
    ...
    {
        if (mode == ConditionGroupMode.Not)
            return conditions == null
                || conditions.Length == 0
                || conditions[0] == null
                || !conditions[0].Evaluate(state);
        if (conditions == null || conditions.Length == 0)
            return mode == ConditionGroupMode.All;
        foreach (Condition condition in conditions)
        {
            bool result = condition == null || condition.Evaluate(state);
            if (mode == ConditionGroupMode.All && !result)
                return false;
            if (mode == ConditionGroupMode.Any && result)
                return true;
        }
        return mode == ConditionGroupMode.All;
    }
